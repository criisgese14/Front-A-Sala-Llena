...
const MySeats = ({seatsNumber, setSeatAvailable, seatsavailable, form}) => {
    ...
    const [available, setAvailable] = useState(showdetail.tickets && showdetail.tickets.length>0? 
        showdetail.tickets.map(el => el.seatNumber): JSON.parse(window.localStorage.getItem('show')))
    ...
    <div>
        <div className={style.component}>
            ...
            <ul className={style.showcase}>
                <li><div className={style.seat}></div><small>Occupied</small></li>
                <li><div className={`${style.seat} ${style.selected}`}></div><small>Selected</small></li>
            </ul>
            :
            <ul className={style.showcase}>
                <li><div className={style.seat}></div><small>N/A</small></li>
                <li><div className={`${style.seat} ${style.selected}`}></div><small>Selected</small></li>
                <li><div className={`${style.seat} ${style.occupied}`}></div><small>Occupied</small></li>
            </ul>}
            <div className={style.container}>
                {filas.map(r =>
                    <div className={style.row}>
                        ...
                        form?
                        sillas.map(el=> 
                            <div className={selected.includes(`${r}-${el}`)?
